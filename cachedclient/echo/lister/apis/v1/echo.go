...
type EchoLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Echo, err error)
...
// List lists all Echos in the indexer.
func (s *echoLister) List(selector labels.Selector) (ret []*v1.Echo, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Echo))
	})
	return ret, err
}
...
// List lists all Echos in the indexer for a given namespace.
func (s echoNamespaceLister) List(selector labels.Selector) (ret []*v1.Echo, err error) {
...
// Get retrieves the Echo from the indexer for a given namespace and name.
func (s echoNamespaceLister) Get(name string) (*v1.Echo, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("echo"), name)
	}
	return obj.(*v1.Echo), nil
}
